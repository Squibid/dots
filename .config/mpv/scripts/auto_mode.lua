...
function on_image_mode_activate()
    ...
    mp.set_property("background", "#1A1A1A")                   -- use dark grey background for images
    mp.command("script-message osc-visibility never no_osd")   -- disable osc for images
    mp.set_property("loop-file", "inf")                     -- use loop-file=no for anything except GIF
    mp.command("script-message minimap-enable")   -- enable minimap
end
...
function on_image_mode_deactivate()
    ...
    mp.command("script-message minimap-disable")   -- disable minimap
end
...
image_mode_bindings = {
    { "k",          function () mp.command("no-osd add video-pan-y  0.02") end,  "repeatable" }, -- move image up
    { "j",          function () mp.command("no-osd add video-pan-y -0.02") end,  "repeatable" }, -- move image down
    { "h",          function () mp.command("no-osd add video-pan-x  0.02") end,  "repeatable" }, -- move image left
    { "l",          function () mp.command("no-osd add video-pan-x -0.02") end,  "repeatable" }, -- move image right
    { "N",          function () mp.command("playlist-prev") end,                 "repeatable" }, -- show previous image
    { "n",          function () mp.command("playlist-next") end,                 "repeatable" }, -- show next image
    { "K",          function () mp.command("add video-zoom  0.1") end,           "repeatable" }, -- increase image size
    { "J",          function () mp.command("add video-zoom -0.1") end,           "repeatable" }, -- decrease image size
